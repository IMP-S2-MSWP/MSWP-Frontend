...
import axios from 'axios';
 ...
 const SignUp=()=>{
  ...
  const  handleNextClick= async()=>{
    ...
    else if(pageIndex === 2 && userData.password !== passwordConfirmation){ // 추가된 부분
...
async function checkUsernameAvailability(username) {
  try {
    ...
    const response = await axios.post('http://192.168.0.16:8080/api/register/validation', {
...
const handleRegister = async () => {
  ...
  try {
    // 서버에 회원가입 요청을 보냄
    const response = await axios.post('http://192.168.0.16:8080/api/register', {
